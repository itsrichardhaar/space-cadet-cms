<script>
  ...
  let min  = $derived(options?.min  ?? undefined);
  let max  = $derived(options?.max  ?? undefined);
  let step = $derived(options?.step ?? undefined);
</script>
...
<div class="field">
  ...
  {/if}
  <input class="input" type="number" bind:value {min} {max} {step} />
</div>
